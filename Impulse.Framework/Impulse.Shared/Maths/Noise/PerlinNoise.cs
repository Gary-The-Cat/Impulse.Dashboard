// <copyright file="PerlinNoise.cs" company="PlaceholderCompany">
...
namespace Impulse.Shared.Maths.Noise;

public class PerlinNoise
{
    private const int GradientSizeTable = 256;
    private readonly Random random;
    private readonly double[] gradients = new double[GradientSizeTable * 3];
    ...
    private readonly byte[] perm = new byte[]
    {
    ...
    public double GetValue(int x, int y, double divisor)
    {
        return Noise(2 * x * divisor, 2 * y * divisor, -0.5) + (1 / 2 * 0.7) +

               // Second octave
               ((Noise(4 * x * divisor, 4 * y * divisor, 0) + 1) / 2 * 0.2) +

               // Third octave
               ((Noise(8 * x * divisor, 8 * y * divisor, +0.5) + 1) / 2 * 0.1);
    }
